...
void phyimp::calculate_dzdt(std::vector<std::complex<double>>& dzdt)
{
    ...
    //NOTE: LU and LLT wouldn't really benefit from n-1 equations, so we choose QR
...
void phyimp::solve_homotopy_continuation(std::vector<std::complex<double>>& z, double t0, double t1)
{
    ...
    auto integrator = odeint::make_controlled<odeint::runge_kutta_fehlberg78<state_type>>(0.0, 1e-9);
    //do integration
    size_t steps = odeint::integrate_adaptive(integrator, difffunc, z, t0, t1, 0.01);
}
